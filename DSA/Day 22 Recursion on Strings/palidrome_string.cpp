...
bool palidromeLoop(string str, int n){
    int s = 0, e = n -1;
    while(s < e){
        if (str[s] != str[e]) return false; // Or return 0
        s++;
        e--;
    }
...
bool palidromeRecursion(string str, int s, int e){
    // base case
    ...
    return palidromeRecursion(str, s, e);
}
...
int main(){
    ...
    string str = "raajr";
    if(palidromeRecursion(str, 0, str.length()-1)){
